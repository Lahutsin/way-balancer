...
- retry or orchestration policy around best-effort fan-out
...
4. Trigger a local purge on one node through `POST /cache/purge`; that node can then fan the bounded invalidation event out to the peer node through signed `POST /cache/invalidate`.
